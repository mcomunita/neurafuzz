...
class FuzzDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.input_files = natsorted(
            glob.glob(os.path.join(self.root_dir, "*input*.wav")))
        self.target_files = natsorted(
            glob.glob(os.path.join(self.root_dir, "*target*.wav")))
        ...
        for iidx, ifile in enumerate(self.input_files):
            ...
            for tidx, tfile in enumerate(self.target_files):
                ...
                if self.preload:
                    sys.stdout.write(
                        f"* Pre-loading... {tidx+1:3d}/{len(self.target_files):3d} ...\r")
                    sys.stdout.flush()
